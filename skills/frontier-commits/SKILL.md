...
- `detect --date <YYYY-MM-DD> [--lookback-days N]` prints **one JSON object** as its final stdout line: `{"run_date", "baseline_date", "already_shipped_this_week", "thin", "stories": [{key, type, org, repo, url, title, score, facts}, ...]}`.
...
- `"slug_prefix": "week-of"` — renames the slug's literal so this show's episodes mint `week-of-<month>-<d>-<yyyy>` (e.g. `week-of-august-24-2026`, echoing the title's "Week of" tail) instead of the daily show's `daily-digest-<date>` (#162). The slug is the `/frontier-commits/<slug>/` permalink and the `isPermaLink` guid, so without this key every frontier permalink and guid carries the daily show's name — and is immutable once published. It stays keyed on the manifest `date` alone (#128): the prefix swaps the literal, it never re-couples the slug to the title.
...
- `reported.json` — story-key → `{date, episode_uri}` mention-once log, and the sole input to `detect`'s same-ISO-week ship guard (step 3). Written by `fc_stories.py mark` **only after the R2 publish succeeds** (the same post-success discipline as `covered.json` — the success condition is the publish, since that is the ship here); malformed → `{}` — worst case a repeated mention one week. The recorded `episode_uri` is the published mp3 URL: this show has no Spotify episode URI, and the URL is the episode's durable identity.
