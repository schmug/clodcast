# Frontier Commits Weekly Run — this is a stub
...
guidance and reporting fields (see `skills/daily-podcast/prompts/daily.md` for
that post-mortem). Duplicating a prompt is the same mistake as duplicating
code, and it fails the same way. So the weekly procedure has exactly one home,
this file points at it, and a drift test (`tests/test_fc_skill_md.py`) goes
red if the procedure ever grows back here.

## If you are a scheduler

Invoke the `frontier-commits` skill and follow its **"Unattended weekly run"**
section. Do not copy those steps into your own prompt — that recreates the
fork. A scheduler's prompt should be a trigger, not a specification:
